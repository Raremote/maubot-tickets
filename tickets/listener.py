...
class TicketsHandler:
    ...
    async def _block_if_intake_room(self, evt: MessageEvent) -> bool:
        ...
        allowed_patterns = [
            "!ticket intake add",
            "!ticket intake remove",
            "!ticket intake list",
            "!ticket intake enable",
            "!ticket intake disable",
            "!ticket intake space",
            "!ticket help",
        ]

        # Also allow the parent command "!ticket intake" (without subcommand)
        if command_text == "!ticket intake":
            return False
    ...
    async def _block_if_command_room_for_intake_commands(self, evt: MessageEvent) -> bool:
        ...
        # Also block the parent command "!ticket intake" (without subcommand)
        if command_text == "!ticket intake":
    ...
    async def _show_help(self, evt: MessageEvent) -> None:
        ...
        response = (
            ...
            "- `!ticket intake` - Manage support intake rooms (add, remove, list, enable, disable, space)\n"
            "- `!ticket command` - Manage command rooms (add, remove, list, enable, disable, space)\n"
    ...
    @ticket_command.subcommand("intake", help="Manage intake rooms")
    async def intake(self, evt: MessageEvent) -> None:
        ...
        await evt.reply(
            ...
            "• `!ticket intake add` - Add this room as a support intake room\n"
            "• `!ticket intake remove [all]` - Remove this room (or all) from support intake rooms\n"
            "• `!ticket intake list` - List all support intake rooms\n"
            "• `!ticket intake enable` - Enable this intake room\n"
            "• `!ticket intake disable` - Disable this intake room\n"
            "• `!ticket intake space [set|unset|unset all|debug|fix]` - Configure ticket space for this intake room (set/unset/unset all/debug/fix/show)\n"
        )
    @intake.subcommand("add", help="Add this room as a support intake room")
    ...
    @intake.subcommand("remove", help="Remove this room (or all) from support intake rooms")
    ...
    @intake.subcommand("list", help="List all support intake rooms")
    ...
    @intake.subcommand("enable", help="Enable this intake room")
    ...
    @intake.subcommand("disable", help="Disable this intake room")
    ...
    @intake.subcommand("space", help="Set, unset, unset all, debug, fix, or show the ticket space for this intake room")
        ...
        if action_lower == "unset" and subaction_arg == "all":
            ...
            if success:
                ...
                if other_rooms_with_space:
                    ...
                    response += "\nTo unset all spaces, use `!ticket intake space unset all` or run `!ticket intake space unset` in those rooms."
                else:
            ...
            if not space_id:
                ...
                response += "\nThen try again: `!ticket intake space set`"
            ...
            if success:
                ...
                response += "4. Use `!ticket intake space debug` to verify configuration\n"
            ...
            current_ticket_space = intake_room.get("space_id")
            if not current_ticket_space:
                await evt.reply("❌ No ticket space configured for this intake room. Use `!ticket intake space set` first.")
                return
            ...
            response += "3. Use `!ticket intake space debug` to verify configuration\n"
            ...
            if this_space:
                if is_room_space:
                    ...
                    response += "You can set this space as the ticket space with: `!ticket intake space set`\n"
                else:
                ...
                response += "1. Add this room to a space (as a child room)\n"
                response += "2. Make this room a space (change room type to space)\n"
                response += "Then use: `!ticket intake space set`\n"

            if this_intake_room_space:
                response += "\nTo unset the ticket space: `!ticket intake space unset` (or `unset all` to unset from all intake rooms)"
    ...
    @ticket_command.subcommand("command", help="Manage command rooms")
    async def command_room(self, evt: MessageEvent) -> None:
        ...
        await evt.reply(
            ...
            "• `!ticket command add` - Mark this room as a command room\n"
            "• `!ticket command remove [all]` - Remove this room (or all) from command rooms\n"
            "• `!ticket command list` - List all command rooms\n"
            "• `!ticket command enable` - Enable this command room\n"
            "• `!ticket command disable` - Disable this command room\n"
            "• `!ticket command space [set|clear|clear all]` - Configure ticket space (set/clear/clear all/show)\n"
        )
    ...
    @command_room.subcommand("space", help="Set, clear, unset, clear all, or debug the ticket space for this command room")
    @command.argument("action", label="action", required=False, pass_raw=True)
        ...
        if action_lower == "clear" and subaction_arg == "all":
            ...
            if success:
                ...
                if other_rooms_with_space:
                    ...
                    response += "\nTo clear all spaces, use `!ticket command space clear all` or run `!ticket command space clear` in those rooms."
                else:
            ...
            if not space_id:
                ...
                if all_parents and not any(p[2] for p in all_parents):
                    response += "\n**If room already has SPACE_PARENT events:**\n"
                    response += "The via field may be empty. You may need to re-add the room to the space.\n"
                response += "\nThen try again: `!ticket command space set`"
            ...
            if success:
                ...
                response += "4. Use `!ticket command space debug` to verify configuration\n"
            ...
            current_ticket_space = command_room.get("space_id")
            if not current_ticket_space:
                await evt.reply("❌ No ticket space configured for this command room. Use `!ticket command space set` first.")
                return
            ...
            response += "1. Grant bot power level ≥50 in the space (Element: Space settings → Roles & Permissions)\n"
            response += "2. New tickets will automatically be added to the space\n"
            response += "3. Use `!ticket command space debug` to verify configuration\n"
            ...
            if current_ticket_space:
                ...
                if rooms_with_space:
                    ...
                    if len(rooms_with_space) > 1:
                        ...
                        response += "To avoid confusion, clear spaces from unused command rooms: `!ticket command space clear` (or `clear all` to clear from all command rooms)\n"
                response += "\n"
            else:
            ...
            if this_space:
                if is_room_space:
                    ...
                    response += "You can set this space as the ticket space with: `!ticket command space set`\n"
                else:
                ...
                response += "1. Add this room to a space (as a child room)\n"
                response += "2. Make this room a space (change room type to space)\n"
                response += "Then use: `!ticket command space set`\n"

            if current_ticket_space:
                response += "\nTo clear the ticket space: `!ticket command space clear` (or `clear all` to clear from all command rooms)"
        ...
        try:
            ...
            if not enabled_intake_rooms:
                await evt.reply(
                    ...
                    "2. Run: `!ticket intake add`\n"
                    "3. Ensure the room is enabled with: `!ticket intake enable`"
                )
                return
        except Exception as e:
    ...
    @ticket_command.subcommand("resolve", help="Resolve the ticket")
    ...
    @ticket_command.subcommand("debug", help="Debug command (staff only)")
